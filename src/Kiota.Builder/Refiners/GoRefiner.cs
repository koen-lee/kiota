...
public class GoRefiner : CommonLanguageRefiner
{
    ...
    private static void AddConstructorsForErrorClasses(CodeElement currentElement)
    {
        ...
        {
            // Add parameterless constructor if not already present (Go uses New* functions)
            if (!codeClass.Methods.Any(static x => x.IsOfKind(CodeMethodKind.Constructor) && !x.Parameters.Any()))
            {
                var parameterlessConstructor = CreateConstructor(codeClass, $"New{codeClass.Name}", "Instantiates a new {TypeName} and sets the default values.");
                codeClass.AddMethod(parameterlessConstructor);
            }

            var messageParameter = CreateErrorMessageParameter("string");
            // Add message constructor if not already present (Go uses NewWithMessage pattern)
            ...
            {
                var messageConstructor = CreateConstructor(codeClass, $"New{codeClass.Name}WithMessage", "Instantiates a new {TypeName} with the specified error message.");
    ...
    private static CodeMethod CreateConstructor(CodeClass codeClass, string methodName, string descriptionTemplate)
    {
        return new CodeMethod
        {
            Name = methodName, // Name is used for documentation, actual name generated by writer
            Kind = CodeMethodKind.Constructor,
            IsAsync = false,
            IsStatic = true, // Go constructors are static factory functions
